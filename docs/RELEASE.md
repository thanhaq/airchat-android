...
The package script runs the full preflight gate, refreshes generated AirChat artifacts in `release/`, copies the `fdroidDebug` APK, writes `SHA256SUMS.txt`, writes `RELEASE_MANIFEST.json`, and drafts release notes. The `release/` folder is ignored so generated artifacts do not enter source control.
...
The signed package script runs the full preflight gate, builds `:app:assembleFdroidRelease`, copies the signed `fdroidRelease` APK into `release/`, writes `SHA256SUMS.txt`, writes `RELEASE_MANIFEST.json`, records the source commit, records `distributionFlavor` and `gradleVariant`, and records the signing certificate SHA-256 fingerprint in `RELEASE_NOTES.md`.
...
Pushing a tag like `v0.1.0` runs `.github/workflows/release.yml`. The workflow runs the full preflight gate, uploads a signed `fdroidRelease` APK when signing secrets are configured, falls back to a `fdroidDebug` test APK otherwise, uploads `SHA256SUMS.txt` and generated notes as an Actions artifact, then creates a draft GitHub Release.
...
- `distributionFlavor` and `gradleVariant` are listed in `RELEASE_MANIFEST.json`.
