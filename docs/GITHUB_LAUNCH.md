...
- Show `/join`, the room switcher with unread counts, `/lock`, matching room codes, QR room-code card, a locked private-room message unlocking after the second phone enters the passphrase, `/rotate`, QR safety card, encrypted DM, file receive/save/share, diagnostics recent events, and background mesh notification.
